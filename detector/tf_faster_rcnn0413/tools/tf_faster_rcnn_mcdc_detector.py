...
MCDC_CLASSES = ('__background__',
                'xiaoxingche', 'huoche', 'gongjiaokeche', 
                'gongchengche', 'sanlunche', 'others', 'unknown', 'none')

NETS = {'vgg16': ('vgg16_faster_rcnn_iter_70000.ckpt',),'res101': 
        ('res101_faster_rcnn_iter_110000.ckpt','res101_faster_rcnn_iter_50000.ckpt', 'res101_faster_rcnn_iter_70000.ckpt')}
DATASETS= {'pascal_voc': ('voc_2007_trainval',),'pascal_voc_0712': ('voc_2007_trainval+voc_2012_trainval',), 
          'mcdc':('mcdc_train_10000', 'MCDC_train_50000_coco+MCDC_train_100000_coco')
          }

def find_middle_car_use_area(boxes,rang=None):
    if rang==None:
        rang=[[710, 350, 1210, 850]]
        rang=np.array(rang)
...
#     print('Detection took {:.3f}s for {:d} object proposals'.format(timer.total_time, boxes.shape[0]))
    ...
    CONF_THRESH = 0.1
    NMS_THRESH = 0.3
    for cls_ind, cls in enumerate(MCDC_CLASSES[1:]):
        if only_car :
            if cls=='xiaoxingche':
...
class TfFasterRcnnDetector:
    def __init__(self, args):
        ...
        tfmodel = os.path.join(cur_file,'..','output', demonet, DATASETS[dataset][1], 'default',
                                  NETS[demonet][2])
        ...
        with self.sess1.as_default():
            ...
            with g1.as_default():
                ...
                self.net.create_architecture("TEST", 9,
