...
def parse_args():
    ...
    parser.add_argument('--n_steps', type=int, help='n_steps', default=50)
    parser.add_argument('--learning_rate', type=float, help='learning_rate', default=0.006)
    parser.add_argument('--is_training', type=bool, help='is_training', default=False)
...
def extract_sample(filename, time_file, pre_file):
...
def try_different_method(clf, x_train, y_train, x_test, y_test):
    ...
    print('relative v error: ', x_err_esti(result, y_test))
    print('mean absolute error: ', vx_err_esti(result, y_test))

def add_layer_tf(inputs, in_size, out_size, activation_function=None ,name=None):
    # add one more layer and return the output of this layer
    initializer = tf.contrib.layers.xavier_initializer()
    # Weights = tf.Variable(initializer, [in_size,out_size])
    Weights = tf.get_variable(name=name, shape=[in_size, out_size ], initializer=initializer)
...
def data_processing(samples, targets, m=10, Tx=50):
    ...
    n_y = targets.shape[-1]
    num = samples.shape[0]
    data_x = np.zeros((m,Tx,n_x), dtype=np.float32)
    data_y = np.zeros((m,Tx,n_y), dtype=np.float32)
    for i in range(m):
        random_idx = np.random.choice(num - Tx)
        data_x[i, :, :] = samples[random_idx:(random_idx + Tx), :]
        data_y[i, :, :] = targets[random_idx:(random_idx + Tx), :]
...
def draw_line(xs, pred, labels, name):
    ...
    plt.plot(xs, pred[0, :, 0].flatten(), 'r', xs, labels[0, :, 0].flatten(), 'b')
    plt.ylim((-2, 20))
#     plt.draw()
#     plt.pause(1)
    plt.savefig(name)
...
if __name__ == '__main__':
    ...
    for file in valid_gt_files:
        ...
        t_samples, t_targets = extract_sample(file, time_file, pre_file)
        # data_x, data_y = data_processing(t_samples, t_targets, M, Tx)
...
#     test_inds = inds
    ...
    if is_training == True:
        ...
        for i in range(20001):
            ...
            if i % 1000 ==0:
                ...
                if x_vx_mode == 'x':
                    ...
                    draw_line(xs, y_test_pred, y_test_batch,name = 'show/x_%d.png'%i)
                elif x_vx_mode == 'vx':
                    print('error for vx:', vx_err_esti(y_test_pred[:, :, 0], y_test_batch[:, :, 0]))
                    draw_line(xs, y_test_pred, y_test_batch, name='show/vx_%d.png' % i)
                elif x_vx_mode == 'x_vx':
                    print('error for x:', x_err_esti(y_test_pred[:, :, 0], y_test_batch[:, :, 0]))
                    print('error for vx:', vx_err_esti(y_test_pred[:, :, 1], y_test_batch[:, :, 1]))
                    draw_x_vx(xs, y_test_pred[0, :, 0].flatten(), y_test_pred[0, :, 1].flatten(),
                              y_test_batch[0,:,0].flatten(),y_test_batch[0,:,1].flatten(),name = "curve_train/x_vx_%d.png"%i)


            if i == 10000:
                sess.run(tf.assign(model.learning_rate, lr*0.1))
        ...
        for ind in range(num_video_test):
            ...
            if x_vx_mode == 'x':
                print('error for x:', x_err_esti(y_test_pred[:, :, 0], y_test_batch[:, :, 0]))
                draw_line(xs, y_test_pred, y_test_batch, name='show/x_%d_test.png' % ind)
            elif x_vx_mode == 'vx':
                print('error for vx:', vx_err_esti(y_test_pred[:, :, 0], y_test_batch[:, :, 0]))
                draw_line(xs, y_test_pred, y_test_batch, name='show/vx_%d_test.png' % ind)
            elif x_vx_mode == 'x_vx':
                print('error for x:', x_err_esti(y_test_pred[:, :, 0], y_test_batch[:, :, 0]))
                print('error for vx:', vx_err_esti(y_test_pred[:, :, 1], y_test_batch[:, :, 1]))
                draw_x_vx(xs, y_test_pred[0, :, 0].flatten(), y_test_pred[0, :, 1].flatten(),
                          y_test_batch[0, :, 0].flatten(), y_test_batch[0, :, 1].flatten(), 
                          name="curve_test/x_vx_%d_test.png"%ind)
